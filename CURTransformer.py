...
def create_mask(window_size, displacement, upper_lower, left_right):
    mask = 	mds.ops.Zeros(window_size ** 2, window_size ** 2)
...
class PatchMerging(nn.Cell):
    def __init__(self, in_channels, out_channels=32, downscaling_factor=1):
        super().__init__()
        # self.conv1 = mds.ops.Conv2D(in_channels, 16, 3, 1, 1)
        # self.relu = nn.ReLU(inplace=True)
        # self.conv2 = mds.ops.Conv2D(16, 32, 3, 1, 1)
        # self.conv3 = mds.ops.Conv2D(32, out_channels*3, 3, 1, 1)
        self.relu = nn.ReLU(inplace=True)
        self.conv1_1 = mds.ops.Conv2D(in_channels, 64, 3, 1, 1)
        self.conv2_1 = mds.ops.Conv2D(64, 32, 3, 1, 1)
        self.conv3_1 = mds.ops.Conv2D(32, out_channels, 3, 1, 1)
        self.conv1_2 = mds.ops.Conv2D(in_channels, 64, 3, 1, 1)
        self.conv2_2 = mds.ops.Conv2D(64, 32, 3, 1, 1)
        self.conv3_2 = mds.ops.Conv2D(32, out_channels, 3, 1, 1)
        self.conv1_3 = mds.ops.Conv2D(in_channels, 64, 3, 1, 1)
        self.conv2_3 = mds.ops.Conv2D(64, 32, 3, 1, 1)
        self.conv3_3 = mds.ops.Conv2D(32, out_channels, 3, 1, 1)
...
class WindowAttention(nn.Cell):
    def __init__(self, in_channels, dim, heads, head_dim, shifted, window_size, relative_pos_embedding):
        ...
        if self.relative_pos_embedding:
            ...
            self.pos_embedding = nn.Parameter(mds.ops.StandardNormal(2 * window_size - 1, 2 * window_size - 1))
        else:
            self.pos_embedding = nn.Parameter(mds.ops.StandardNormal(window_size ** 2, window_size ** 2))
...
class merage(nn.Cell):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv = mds.ops.Conv2D(in_channels, out_channels, kernel_size=3, stride=1,
...
class ResBlock(nn.Cell):
    ...
    def __init__(self, channels):
        ...
        self.conv1 = mds.ops.Conv2D(channels, channels, kernel_size=5, stride=1,
                               padding=2, bias=False)
        # self.bn1 = nn.BatchNorm2d(channels)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = mds.ops.Conv2D(channels, channels, kernel_size=5, stride=1,
                               padding=2, bias=False)
...
class Head(nn.Cell):
    ...
    def __init__(self, in_channels, out_channels=64, channels=32):
        ...
        self.conv1 = mds.ops.Conv2D(in_channels, out_channels, kernel_size=3, stride=1,
                               padding=1, bias=False)
        ...
        self.conv2 = mds.ops.Conv2D(out_channels, channels, kernel_size=1, stride=1,
...
class CURTransformer(nn.Cell):
    def __init__(self, *, hidden_dim, layers, heads, channels=32, num_classes=1000, head_dim=8, window_size=28,
        ...
        self.tailsets = nn.Sequential(mds.ops.Conv2D(3, 16, kernel_size=3, stride=1,
                           padding=1, bias=False),
                                      nn.ReLU(inplace=True),
                                      mds.ops.Conv2D(16, 3, kernel_size=3, stride=1,
                                                padding=1, bias=False)
                                      )
        ...
        self.conv2 = mds.ops.Conv2D(channels, 3, 3, 1, 1)
